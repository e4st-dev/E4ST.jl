...
SUITE["load_demand_table!"] = @benchmarkable E4ST.load_demand_table!(config, data) setup=(config=make_random_inputs(demand_shape_file=false); data = load_data(config))
SUITE["load_demand_table! with shape"] = @benchmarkable E4ST.load_demand_table!(config, data) setup=(config=make_random_inputs(demand_shape_file=true); data = load_data(config))
SUITE["load_demand_table! with match"] = @benchmarkable E4ST.load_demand_table!(config, data) setup=(config=make_random_inputs(demand_match_file=true); data = load_data(config))
...
SUITE["setup_dcopf!"] = @benchmarkable setup_dcopf!(config, data, model) setup=(config=make_random_inputs(); data=load_data(config); model = JuMP.Model(HiGHS.Optimizer))