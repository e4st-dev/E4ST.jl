...
function add_results_formula!(data, table_name::Symbol, result_name::Symbol, formula::String, unit::Type{<:Unit}, description::String)
    ...
    if startswith(formula, "sum")
        formula_stripped = formula[4:end]
        ...
        isderived = false
        fn = identity # TODO maybe compute something a bit smarter here.
    else
        ...
        isderived = true
        fn_string = string("row->", replace(formula, r"(\w+)"=>s"row.\1", r"[^.]([\+\*\/\-])"=>s".\1"))
...
function compute_result(data, table_name, result_name, idxs=(:), yr_idxs=(:), hr_idxs=(:))
    ...
    if res_formula.isderived === false
        ...
        return sum_result(data, table, dep_cols..., _idxs, _yr_idxs, _hr_idxs)::Float64
    else
        # Recursive
        dep_cols = res_formula.dependent_columns
        fn = res_formula.fn
...
function sum_result(data, table::DataFrame, col1::Symbol, idxs, yr_idxs, hr_idxs)
    _sum_result(table[!, col1], idxs, yr_idxs, hr_idxs)
end
function sum_result(data, table::DataFrame, col1::Symbol, col2::Symbol, idxs, yr_idxs, hr_idxs)
    _sum_result(table[!, col1], table[!, col2], idxs, yr_idxs, hr_idxs)
end
function sum_result(data, table::DataFrame, col1::Symbol, col2::Symbol, col3::Symbol, idxs, yr_idxs, hr_idxs)
    _sum_result(table[!, col1], table[!, col2], table[!, col3], idxs, yr_idxs, hr_idxs)
end

function _sum_result(v1, idxs, yr_idxs, hr_idxs)
    sum(_getindex(v1, i, y, h) for i in idxs, y in yr_idxs, h in hr_idxs)
end
function _sum_result(v1, v2, idxs, yr_idxs, hr_idxs)
    sum(_getindex(v1, i, y, h)*_getindex(v2, i, y, h) for i in idxs, y in yr_idxs, h in hr_idxs)
end
function _sum_result(v1, v2, v3, idxs, yr_idxs, hr_idxs)
