...
function summarize_config()
    ...
    push!(df, 
        ...
        (:line_loss_type, false, "pflow_in", "The term in the power balancing equation that gets penalized with line losses.  Can be \"pflow_in\" or \"pserv\". Using \"pflow_in\" is more accurate in that it accounts for only losses on power coming from somewhere else, at the expense of a larger problem size and greater solve time."),
    )
    return df
end
