...
* `gen_filters::OrderedDict{Symbol}` - filters for qualifying generators, stored as an OrderedDict with gen table columns and values (`:emis_co2=>"<=0.1"` for co2 emission rate less than or equal to 0.1)
"""
struct CostAdder <: Modification
    name::Symbol
    cost_type::Symbol
    ...
    gen_filters::OrderedDict{Symbol}
...
function E4ST.modify_setup_data!(m::CostAdder, config, data)
    ...
    row_idxs = get_row_idxs(table, parse_comparisons(m.gen_filters))

    @info "Applying CostAdder $(m.name) to $(length(row_idxs)) rows of the $(m.table_name) table"

    years = get_years(data)
    unit = m.cost_type == :variable ? DollarsPerMWh : DollarsPerMWCapacityPerHour
    ...
    for row_idx in row_idxs
        cost = ByYear([get(m.values, yr, 0.0) for yr in years])
        costs[row_idx] = cost
    end
end
...
function E4ST.modify_model!(m::CostAdder, config, data, model)
    ...
    make_cost_adder_exp!(m, config, data, model)        
